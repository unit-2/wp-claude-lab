...
- 「カバレッジ 80%+」（下記）はテーマ・プラグインのビジネスロジック層に適用される目標であり、
  基盤スクリプトについては**ブロック系フック（`exit 2` を持つもの）にスモークテスト
  （正常系・代表的な異常系）があること**を基準とする。`post-php-security.sh` /
  `file-changed-security.sh` のように警告・注入のみで常に `exit 0` するフックは対象外
  （ブロック判定がなく、壊れても品質ゲートとして機能を失わないため）
