...
- ✅ **コミットは素通りさせない** — phpstan + レビュー確認を通過しないと `git commit` が拒否される
...
現状のフラグは `touch` による単純なオン/オフで、有効期限やコミット対象との紐付けはまだありません。
これは設計・承認済みで、より厳密な「TTL付きレビューゲート」への強化を予定しています
（`docs/plans/hooks-design.md` §4-A参照。実装は未着手）。
...
必須環境: [Local by Flywheel](https://localwp.com/) など任意のローカル WordPress 環境、
