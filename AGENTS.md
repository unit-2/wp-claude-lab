...
セットアップ手順は README.md の「クイックスタート」節を参照。
...
- **スキル**＝知識（必要なときに読む手順書）
- **フック**＝強制（機械的に判定できる違反を自動ブロック・警告）
- **エージェント**＝隔離された実行主体（独立したコンテキストで調査・レビューし、要約だけを返す）

サブエージェントは `.claude/agents/` に2体定義済み:
- `adversarial-code-reviewer` — コミット前・PHP実装完了時の敵対的コードレビュー（修正はしない）
- `security-reviewer` — セキュリティ関連コード（認証・入力・DB・REST API等）のレビュー（修正はしない）
...
1. `adversarial-reviewer` スキルでコードをレビューする
2. `bash bin/wp-review.sh <target>` を実行する（phpstan → レビューチェックリスト → 確認フラグの付与を一括実行）
3. `git commit` を実行する
